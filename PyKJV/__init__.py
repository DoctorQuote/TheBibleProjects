tables = '''CREATE TABLE IF NOT EXISTS SqlTblVerse (ID Integer PRIMARY KEY AUTOINCREMENT, BookID int, BookChapterID int, BookVerseID int, Verse String, VerseType int);
CREATE TABLE IF NOT EXISTS SqlNotes (ID Integer PRIMARY KEY AUTOINCREMENT, vStart int, vEnd int, kwords String, Subject String, Notes String, NextId int);
CREATE TABLE IF NOT EXISTS SqlBooks (ID Integer PRIMARY KEY AUTOINCREMENT, Book String, BookMeta String);'''.split('\n')
...
def destroy_everything():
    ...
    tnames = ['SqlTblVerse','SqlBooks','SqlNotes']
    dao = SierraDAO.GetDAO()
    for table in tnames:
        dao.conn.execute(f'DROP TABLE IF EXISTS {table};')
    for table in tables:
        dao.conn.execute(table)
